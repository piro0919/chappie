...
The codebase splits roughly into a Rust backend (`src-tauri/src/`, ~18.8k lines across ~50 top-level modules plus ~18 MCP submodules under `src/mcp/`) handling audio capture, LLM HTTP, system integration, and Tauri commands; and a React renderer (`src/`, ~9k lines) handling wake-word detection, the conversation loop, settings, and HUD / settings windows.
...
- **Opt-in usage analytics with quota incentive**: at the end of every chat turn, [openai.rs::chat_complete](src-tauri/src/openai.rs) calls [analytics::report_turn](src-tauri/src/analytics.rs) with the utterance text, the names of tools the LLM called, lang/mode/latency/success. When consent is OFF the call no-ops at the network layer but still appends to the in-memory ring so the Settings UI can preview "what would be sent." Free-tier users who opt in get +10 daily quota (5 → 15) — the proxy at [lp/src/app/api/chat/route.ts](lp/src/app/api/chat/route.ts) reads `X-Chappie-Analytics-Consent: true` and bumps `effectiveLimit`. The consent header is advisory at the quota layer; the [/api/analytics](lp/src/app/api/analytics/route.ts) ingestion endpoint re-verifies against `analytics_consent` in Supabase, so faking the header gets you the bonus but no data flow. Audio recordings and LLM responses are NEVER captured — only the Whisper transcript text and tool names. 90-day retention via pg_cron. Schema: [lp/supabase/migrations/20260520081650_create_analytics.sql](lp/supabase/migrations/20260520081650_create_analytics.sql).
- **Voice → visual impact is a deliberate differentiation axis**: Alexa / Google Home / Siri are speaker-only devices and can't make "something visually change" happen on a screen. Chappie is a Mac tray app, so it CAN — and that's the strongest moat against established voice assistants. First realization is [src-tauri/src/wallpaper.rs](src-tauri/src/wallpaper.rs): "壁紙を森に変えて" → fetch via the [/api/wallpaper](lp/src/app/api/wallpaper/route.ts) Pixabay proxy → osascript sets `picture of every desktop` with a different photo per monitor. No explicit HUD pill — the wallpaper change itself is the visual confirmation, so the tool just returns success/failure and the LLM's spoken reply (or HUD routing when muted) handles messaging. Image source is Pixabay rather than Unsplash because Unsplash's demo tier is 50 req/hr per *application* (shared across all chappie users) and getting production approval takes days; Pixabay's verified key gives 100 req/min and is good enough for "おしゃれな壁紙" use cases. The API key lives server-side on chappie.kkweb.io, the desktop only sees image URLs which it re-verifies against `pixabay.com` / `cdn.pixabay.com` before downloading. Future tools in this axis: NASA APOD, Wikipedia On This Day, wallpaper rotation (proactive.rs extension), scheduled briefings. Voice → text-only tools (jokes, trivia, advice slip) are *secondary* — useful but not differentiating.
...
- **Character affinity (育成) is felt, not authored** ([src/lib/affinity.ts](src/lib/affinity.ts), v0.19.0): a retention feature where each character (chappie's own voice + each VOICEVOX speaker) grows closer with use. The hard part was avoiding a content explosion (tone × character × 9 langs) — so we DON'T author per-stage text. Instead a tiny per-character counter (`affinity.json`: distinct conversation days, last day, turn count) feeds a continuous score `1 - exp(-days/30)`, and `buildAffinityStance()` turns that single number into one parametric stance prompt ("今の親しさは 0.50 …") that the LLM uses to set its own warmth. Deliberate choices: (1) **continuous value, not bands** — the score moves only ~daily so tone drifts gradually and is stable within a session, which also means no banding was needed for consistency; (2) **no decay** — skipping days never lowers the score; absence is a SEPARATE `daysSinceLast` signal that only flavors the re-greeting ("拗ね"), never the bond; (3) **knowledge vs bond split** — long-term memory / tool-routing (what Chappie knows about *you*) stays shared across characters; only the *bond* is per-character; (4) the stance is injected into the per-turn override (a late, dynamic message right before the user turn), NOT index 0, so the static-persona prompt cache is preserved; (5) **weak models need concrete anchors** — Gemini Flash ignored an abstract "be proportionally casual" knob and also got cancelled by `CHAPPIE_RESET`'s "return to chappie's flat tone"; it only obeyed once the stance had explicit 0/0.5/1 anchors + a です/ます gate AND `CHAPPIE_RESET` was reworded to defer tone to affinity. Always on, no setting, no meter — by design the change is a slow discovery, which is also why it's intentionally NOT advertised on the LP (no visible progression to point at yet).
- **Proactive "flavoring" LLM calls must not carry tools** ([dispatch.rs](src-tauri/src/llm/dispatch.rs) `no_tools`): the VOICEVOX persona-rewrite and idle-chatter calls reuse `chat_complete`, which normally attaches all tools AND executes whatever the model calls. The model read a calendar pre-warning ("15分後に〇〇です") as a request and fired `set_timer` — so a real timer spun up after the warning and re-spoke at the event's start. `chat_complete` now takes a `no_tools` flag that drops the `tools`/`tool_choice` fields and skips context injection; the renderer sets it for these text-only calls (`ChatClient.complete(..., { noTools: true })`).
- **External-mic output mode is a 3-way, not a toggle** ([settings.ts](src/lib/settings.ts) `externalMicOutputMode`): while another app is capturing the mic (a call / recording), Chappie can `voice` (normal), `hud` (stay silent, show on screen), or `silent` (no voice, no HUD). Detection is per-process via CoreAudio ([mic_activity.rs](src-tauri/src/mic_activity.rs), macOS 14+, our own PID excluded — the device-level "is running somewhere" check can't work because our capture stream is always live). All TTS routing funnels through `resolveOutputMode()` in [tauri-bridge.ts](src/lib/tauri-bridge.ts) (system-mute OR external-mic mode). Default `voice` (opt-in) because an app holding the mic open while idle would otherwise keep Chappie quiet.
