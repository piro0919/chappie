...
- **Two run modes** (Settings → radio): **Free** routes through the chappie.kkweb.io proxy (Gemini 2.5 Flash, 5 req/day/device, no API key needed) and **API-key mode (BYOK)** uses the user's own provider key directly. Fresh installs default to Free; existing users with a saved key migrate to BYOK so behavior stays identical. Proxy URL overridable via `CHAPPIE_PROXY_URL` for local dev.
